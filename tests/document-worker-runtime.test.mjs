...
test("document providers work in Workers and never forward credentials on redirects", async () => {
  ...
  const mf = new Miniflare({
    ...
    outboundService: async request => {
      ...
      if (request.headers.get("x-ms-range") === "bytes=0-0") return new Response(new Uint8Array([1]), { status: 206, headers: { etag: '"0xABC"', "content-range": "bytes 0-0/3", "content-length": "1", "x-ms-meta-sha256": "a".repeat(64) } });
