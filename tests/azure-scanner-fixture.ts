...
export function scannerFixture(initialResult = "No threats found") {
  ...
  const transport = (async (input, init) => {
    ...
    if (method === "GET" && headers.get("x-ms-range") === "bytes=0-0") return new Response(new Uint8Array([1]), { status: 206, headers: { etag: blob.etag, "content-range": `bytes 0-0/${blob.size}`, "content-length": "1", "x-ms-meta-sha256": blob.sha256 } });
