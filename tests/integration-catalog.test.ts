...
test("the shared provider gate matches the implemented Lightspeed staging boundary", () => {
...
test("only built pilots may claim that credentials are the remaining connection prerequisite", () => {
  ...
  assert.deepEqual(credentialReady.map((provider) => provider.id).sort(), ["lightspeed", "lightspeed-r"]);
