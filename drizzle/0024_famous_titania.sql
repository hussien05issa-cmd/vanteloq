...
ALTER TABLE `integration_connections` ADD `sync_version` integer DEFAULT 0 NOT NULL;--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `commerce_customers`.`organization_id`
		AND `provider` = `commerce_customers`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `commerce_payments`.`organization_id`
		AND `provider` = `commerce_payments`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `commerce_products`.`organization_id`
		AND `provider` = `commerce_products`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `commerce_sale_lines`.`organization_id`
		AND `provider` = `commerce_sale_lines`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `commerce_suppliers`.`organization_id`
		AND `provider` = `commerce_suppliers`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `integration_location_mappings`.`organization_id`
		AND `provider` = `integration_location_mappings`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `integration_staged_financial_records`.`organization_id`
		AND `provider` = `integration_staged_financial_records`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `integration_staged_sales`.`organization_id`
		AND `provider` = `integration_staged_sales`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `integration_sync_runs`.`organization_id`
		AND `provider` = `integration_sync_runs`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `integration_webhook_events`.`organization_id`
		AND `provider` = `integration_webhook_events`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
SET `connection_id` = COALESCE((SELECT `id` FROM `integration_connections`
	WHERE `organization_id` = `integration_oauth_states`.`organization_id`
		AND `provider` = `integration_oauth_states`.`provider`
	ORDER BY `created_at`, `id` LIMIT 1), `connection_id`);--> statement-breakpoint
...
WHERE EXISTS (
	SELECT 1 FROM `integration_location_mappings`
	INNER JOIN `integration_connections`
		ON `integration_connections`.`id` = `integration_location_mappings`.`connection_id`
		AND `integration_connections`.`organization_id` = `integration_location_mappings`.`organization_id`
	WHERE `integration_location_mappings`.`organization_id` = `inventory_balances`.`organization_id`
...
CREATE INDEX `inventory_balances_workspace_source_idx` ON `inventory_balances` (`organization_id`,`source_connection_id`);--> statement-breakpoint
...
CREATE INDEX `purchase_order_lines_product_idx` ON `purchase_order_lines` (`organization_id`,`provider`,`connection_id`,`external_product_ref`);--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `__new_workspace_documents`("id", "organization_id", "document_type", "file_name", "object_key", "content_type", "size_bytes", "sha256_hex", "security_state", "status", "scan_status", "scanned_at", "scan_provider", "extraction_status", "extracted_json", "uploaded_by_user_id", "created_at", "updated_at") SELECT "id", "organization_id", "document_type", "file_name", "object_key", "content_type", "size_bytes", "sha256_hex", "security_state", "status", 'pending', NULL, NULL, "extraction_status", "extracted_json", "uploaded_by_user_id", "created_at", "updated_at" FROM `workspace_documents`;--> statement-breakpoint
