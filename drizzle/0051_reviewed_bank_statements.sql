...
--> statement-breakpoint
CREATE TRIGGER bank_statement_import_guard BEFORE INSERT ON bank_statement_imports BEGIN
 SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM workspace_documents d WHERE d.id=NEW.document_id AND d.organization_id=NEW.organization_id AND d.security_state='clean' AND d.scan_status='clean' AND d.status IN ('review_required','uploaded') AND d.document_type IN ('bank_statement','other')) THEN RAISE(ABORT,'STATEMENT_DOCUMENT_UNAVAILABLE') END;
 SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM bank_accounts b JOIN financial_accounts a ON a.id=b.financial_account_id AND a.organization_id=b.organization_id WHERE b.id=NEW.bank_account_id AND b.organization_id=NEW.organization_id AND b.financial_account_id=NEW.financial_account_id AND b.provider='manual' AND b.connection_status='manual' AND b.account_type IN ('chequing','savings','merchant') AND a.active=1 AND a.account_type='asset' AND UPPER(b.currency)=NEW.currency AND b.demo_record=NEW.demo_record) THEN RAISE(ABORT,'STATEMENT_ACCOUNT_UNAVAILABLE') END;
 SELECT CASE WHEN EXISTS (SELECT 1 FROM bank_statement_imports s WHERE s.organization_id=NEW.organization_id AND s.bank_account_id=NEW.bank_account_id AND s.start_date<=NEW.end_date AND s.end_date>=NEW.start_date) THEN RAISE(ABORT,'STATEMENT_PERIOD_OVERLAP') END;
END;
--> statement-breakpoint
CREATE TRIGGER bank_statement_approve_guard BEFORE UPDATE OF status ON bank_statement_imports WHEN NEW.status='approved' BEGIN
 SELECT CASE WHEN (SELECT COUNT(*) FROM bank_statement_rows WHERE import_id=NEW.id) <> NEW.row_count THEN RAISE(ABORT,'STATEMENT_ROWS_INCOMPLETE') END;
 SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM workspace_documents d WHERE d.id=NEW.document_id AND d.organization_id=NEW.organization_id AND d.security_state='clean' AND d.scan_status='clean' AND d.status IN ('review_required','uploaded')) THEN RAISE(ABORT,'STATEMENT_DOCUMENT_UNAVAILABLE') END;
 SELECT CASE WHEN EXISTS (SELECT 1 FROM bank_statement_rows r JOIN financial_transactions t ON t.id=r.transaction_id WHERE r.import_id=NEW.id AND (t.organization_id<>NEW.organization_id OR t.account_id<>NEW.financial_account_id OR t.currency<>NEW.currency OR t.demo_record<>NEW.demo_record OR t.source_system<>'bank_statement' OR t.journal_entry_id IS NOT NULL OR t.posting_date<NEW.start_date OR t.posting_date>NEW.end_date)) THEN RAISE(ABORT,'STATEMENT_ROWS_INVALID') END;
 SELECT CASE WHEN (SELECT COALESCE(SUM(CASE WHEN t.amount_cents>0 THEN t.amount_cents ELSE 0 END),0) FROM bank_statement_rows r JOIN financial_transactions t ON t.id=r.transaction_id WHERE r.import_id=NEW.id)<>NEW.inflow_cents OR (SELECT COALESCE(SUM(CASE WHEN t.amount_cents<0 THEN -t.amount_cents ELSE 0 END),0) FROM bank_statement_rows r JOIN financial_transactions t ON t.id=r.transaction_id WHERE r.import_id=NEW.id)<>NEW.outflow_cents THEN RAISE(ABORT,'STATEMENT_TOTALS_INVALID') END;
END;
--> statement-breakpoint
CREATE TRIGGER bank_statement_manual_identity_guard BEFORE INSERT ON bank_accounts WHEN NEW.provider='manual' AND length(NEW.masked_number)>=4 BEGIN
 SELECT CASE WHEN EXISTS (SELECT 1 FROM bank_accounts b WHERE b.organization_id=NEW.organization_id AND lower(trim(b.institution_name))=lower(trim(NEW.institution_name)) AND substr(b.masked_number,-4)=substr(NEW.masked_number,-4) AND b.currency=NEW.currency AND b.demo_record=NEW.demo_record) THEN RAISE(ABORT,'STATEMENT_ACCOUNT_EXISTS') END;
END;
--> statement-breakpoint
...
BEGIN
 SELECT CASE WHEN (SELECT COUNT(*) FROM bank_statement_rows WHERE import_id=OLD.id)<>OLD.row_count THEN RAISE(ABORT,'STATEMENT_UNDO_REVIEW_REQUIRED') END;
 SELECT CASE WHEN EXISTS(SELECT 1 FROM bank_statement_rows r JOIN financial_transactions t ON t.id=r.transaction_id WHERE r.import_id=OLD.id AND (t.organization_id<>OLD.organization_id OR t.account_id<>OLD.financial_account_id OR t.currency<>OLD.currency OR t.demo_record<>OLD.demo_record OR t.posting_date<OLD.start_date OR t.posting_date>OLD.end_date OR t.source_system<>'bank_statement' OR t.source_state<>'posted' OR t.journal_entry_id IS NOT NULL OR t.reconciliation_status<>'unreconciled' OR EXISTS(SELECT 1 FROM bookloq_transaction_matches m WHERE m.transaction_id=t.id AND m.organization_id=OLD.organization_id AND m.status='confirmed'))) THEN RAISE(ABORT,'STATEMENT_UNDO_REVIEW_REQUIRED') END;
 SELECT CASE WHEN (SELECT COALESCE(SUM(CASE WHEN t.amount_cents>0 THEN t.amount_cents ELSE 0 END),0) FROM bank_statement_rows r JOIN financial_transactions t ON t.id=r.transaction_id WHERE r.import_id=OLD.id)<>OLD.inflow_cents OR (SELECT COALESCE(SUM(CASE WHEN t.amount_cents<0 THEN -t.amount_cents ELSE 0 END),0) FROM bank_statement_rows r JOIN financial_transactions t ON t.id=r.transaction_id WHERE r.import_id=OLD.id)<>OLD.outflow_cents THEN RAISE(ABORT,'STATEMENT_UNDO_REVIEW_REQUIRED') END;
