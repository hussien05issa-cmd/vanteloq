...
export function retailDemo(location = "all", missingCost = false) {
  const period = parseCommercePeriod("2026-08-15", "2026-09-11"), now = Date.parse("2026-09-11T19:00:00Z");
  const lines: RetailLine[] = [], stock: RetailStock[] = [], measurements: RetailMeasurement[] = [];
  const outlets = location === "all" ? ["central", "riverside"] : [location];
  for (const outlet of outlets) {
    for (let day = 0; day < 56; day++) {
      const date = businessDateOffset(period.comparisonFrom, day), current = day >= 28;
      for (let basket = 0; basket < (current ? 6 : 8); basket++) {
        const chosen = basket % 3 === 0 ? [0, 1, 2] : basket % 3 === 1 ? [2, 3] : [4, 5];
        for (const item of chosen) {
          const product = catalogue[item], quantity = item === 2 ? 2 : 1, discount = current && basket % 2 === 0 ? Math.round(product.price * quantity * .2) : 0;
          lines.push({ provider: "sample-pos", connectionId: "northline-demo", saleId: outlet + "-" + day + "-" + basket, lineId: product.sku, productRef: product.sku, sku: product.sku, name: product.name, category: product.category, itemType: product.itemType,
            ...
            netCents: product.price * quantity - discount, discountCents: discount, costCents: missingCost && item === 0 ? null : product.cost * quantity });
        }
      }
    }
    for (const product of catalogue) {
      const onHand = product.sku === "VIT-01" ? 320 : product.sku === "CRE-01" ? 8 : 80;
    ...
    for (let buyer = 0; buyer < 12; buyer++) measurements.push({ kind: "loyalty", provider: "sample-pos", connectionId: "northline-demo", outletRef: "", reference: outlet + "-buyer-" + buyer, from: "", to: "", source: "Fictional enrollment ledger", values: { memberSince: buyer < 6 ? "2026-07-01" : "2026-09-01" } });
  }
  return buildRetailIntelligence({ lines, stock, measurements, period, timeZone: "America/Edmonton", asOfDate: period.to, now,
    lots: outlets.map(outlet => ({ name: "Protein bar · chocolate", sku: "BAR-01", locationRef: outlet, expirationDate: "2026-09-18", quantity: 12, costCents: 180 })) });
}
