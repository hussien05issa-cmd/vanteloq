...
export async function requireActiveWorkspaceSession(context: AccessContext, now = Date.now()) {
  ...
  const row = await getD1().prepare(`INSERT INTO workspace_sessions
    ...
    VALUES (?, ?, ?, ?, ?, ?, 0)
    ON CONFLICT(id) DO UPDATE SET last_seen_at = MAX(workspace_sessions.last_seen_at, excluded.last_seen_at)
    ...
    .bind(id, context.userId, context.organizationId, now, now, now + SESSION_MAX_MS, now, now - SESSION_IDLE_MS)
