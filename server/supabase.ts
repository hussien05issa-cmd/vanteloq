...
function authAdminConfiguration(env: VanteloqRuntimeEnv = getRuntimeEnv()) {
  const url = safeUrl(clean(env.SUPABASE_URL));
  const secretKey = clean(env.SUPABASE_SECRET_KEY);
  if (!url || !secretKey) throw new Error("Supabase account deletion is not configured.");
  return { url, secretKey };
}

export function supabaseAccountDeletionReadiness(env: VanteloqRuntimeEnv = getRuntimeEnv()) {
  try {
    authAdminConfiguration(env);
...
export async function revokeSupabaseSessions(request: Request, fetcher: typeof fetch = fetch) {
  const authorization = request.headers.get("authorization")?.trim() ?? "";
  const publishableKey = clean(getRuntimeEnv().SUPABASE_PUBLISHABLE_KEY);
  const url = safeUrl(clean(getRuntimeEnv().SUPABASE_URL));
  if (!url || !publishableKey || !authorization.startsWith("Bearer ")) return false;
  const response = await fetcher(`${url}/auth/v1/logout?scope=global`, {
    method: "POST",
...
export async function deleteSupabaseAuthUser(subject: string, fetcher: typeof fetch = fetch) {
  if (!/^[0-9a-f-]{32,40}$/i.test(subject)) return false;
  const { url, secretKey } = authAdminConfiguration();
  const response = await fetcher(`${url}/auth/v1/admin/users/${encodeURIComponent(subject)}`, {
    method: "DELETE",
    headers: {
      apikey: secretKey,
