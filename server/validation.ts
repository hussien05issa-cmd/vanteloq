...
function selected<T extends readonly string[]>(
  value: unknown,
  allowed: T,
  label: string,
): T[number] {
...
function validWebsite(value: unknown): string {
  ...
  try {
    ...
    throw new ApiError(
      400,
      "INVALID_FIELD",
      "Enter a valid website URL beginning with https://.",
    );
  }
}
...
function isoCountry(value: unknown): string {
  if (
    typeof value !== "string" ||
    !ISO_COUNTRY_CODES.includes(value.toUpperCase())
  ) {
    throw new ApiError(400, "INVALID_FIELD", "Select a valid ISO country.");
  }
  return value.toUpperCase();
}

function validTimezone(value: unknown): string {
  const timezone = requiredString(value, "IANA timezone", 80);
  try {
...
function validCurrency(value: unknown): string {
  const currency = requiredString(value, "ISO currency", 3).toUpperCase();
  ...
  try {
    ...
    throw new ApiError(
      400,
      "INVALID_FIELD",
      "Enter a valid ISO currency code.",
    );
  }
}
...
function businessHours(value: unknown): BusinessHour[] {
  if (!Array.isArray(value) || value.length !== 7) {
    throw new ApiError(
      400,
      "INVALID_HOURS",
      "Provide business hours for every day of the week.",
    );
  }
  ...
  if (seen.size !== 7)
    throw new ApiError(400, "INVALID_HOURS", "Provide each day exactly once.");
  return normalized;
}
...
export function onboardingInput(value: Record<string, unknown>) {
  ...
  if (!EMAIL.test(businessEmail))
    throw new ApiError(400, "INVALID_FIELD", "Enter a valid business email.");
  const phone = optionalString(value.phone, "phone number", 30);
  if (!PHONE.test(phone))
    throw new ApiError(400, "INVALID_FIELD", "Enter a valid phone number.");
  const country = isoCountry(value.country ?? "CA");
  const postalCode = requiredString(
    value.postalCode,
    country === "CA"
      ? "postal code"
      : country === "US"
  ...
  if (!validPostal)
    throw new ApiError(
      400,
      "INVALID_FIELD",
      `Enter a valid ${country === "CA" ? "Canadian postal code" : "U.S. ZIP code"}.`,
    );
  const province =
    country === "CA"
      ? selected(value.province, canadianRegions, "province or territory")
      : country === "US"
        ? selected(value.province, usRegions, "state or territory")
        : requiredString(value.province, "administrative area", 80);
  const sourceMode = selected(
    value.sourceMode ?? "connect_later",
    ["connect_later", "csv", "live"] as const,
    "data source mode",
  );
  ...
  if (sourceMode === "live" && !selectedPos)
    throw new ApiError(
      400,
      "INVALID_FIELD",
      "Select a POS provider to connect.",
    );
...
export function taskCreateInput(value: Record<string, unknown>) {
  ...
  return {
    ...
    sourceType: selected(
      ...
      ["manual", "insight", "alert", "decision"] as const,
      "task source",
    ),
    sourceRef:
      optionalString(value.sourceRef, "task source reference", 120) || null,
...
function dailyMetricRow(value: unknown) {
  if (!value || Array.isArray(value) || typeof value !== "object") {
    throw new ApiError(
      400,
      "INVALID_ROW",
      "Each daily metric row must be an object.",
    );
  }
  ...
  const grossSalesCents = integerValue(
    row.grossSalesCents,
    "gross sales amount",
    0,
  );
  ...
  if (netSalesCents > grossSalesCents) {
    throw new ApiError(
      400,
      "INVALID_FIELD",
      "Net sales cannot exceed gross sales.",
    );
  }
  return {
    businessDate,
    locationRef:
      optionalString(row.locationRef, "location reference", 80) || "all",
    grossSalesCents,
    netSalesCents,
    ...
    cashBalanceCents: nullableInteger(
      row.cashBalanceCents,
      "cash balance",
      -1_000_000_000_000,
    ),
    accountsPayableCents: nullableInteger(
      row.accountsPayableCents,
      "accounts payable",
      0,
    ),
  };
...
export function dailyMetricImportInput(value: Record<string, unknown>) {
  ...
  if (
    ...
    throw new ApiError(
      400,
      "INVALID_ROWS",
      "Provide between 1 and 366 daily metric rows per import.",
    );
  }
  ...
  for (const row of rows) {
    ...
    if (keys.has(key))
      throw new ApiError(
        400,
        "DUPLICATE_ROW",
        "An import cannot repeat the same date and location.",
      );
    keys.add(key);
  }
...
export function businessEventCreateInput(value: Record<string, unknown>) {
  ...
  if (!DATE.test(eventDate) || (reviewDate && !DATE.test(reviewDate))) {
    throw new ApiError(
      400,
      "INVALID_FIELD",
      "Enter valid event and review dates.",
    );
  }
  if (reviewDate && reviewDate < eventDate) {
    throw new ApiError(
      400,
      "INVALID_FIELD",
      "The review date cannot be before the event date.",
    );
  }
  return {
    eventType: selected(
