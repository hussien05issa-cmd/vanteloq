...
import { fetchMonerisPayments, MONERIS_PROVIDER, normalizeMonerisPayment } from "../../integrations/moneris";
...
export async function runSync(request: Request, requestId: string, context: SyncContext, input: Record<string, unknown>, trigger: SyncTrigger) {
    ...
    try {
      ...
      const cursorAfter: MonerisCursor = warnings ? cursorBefore : { nextDay: new Date(Math.min(day.getTime(), today.getTime())).toISOString().slice(0, 10), providerCursor };
      ...
      await renewIntegrationSyncLease(lease);
      const updated = await getDb().update(integrationConnections).set({ lastSuccessfulSyncAt: completedAt, lastSyncCursor: cursorJson, dataPromotionStatus: connection.dataPromotionStatus, lastErrorCode: warnings ? "MONERIS_PAYMENT_ROWS_SKIPPED" : null, syncLeaseOwner: null, syncLeaseExpiresAt: null, updatedAt: completedAt }).where(and(eq(integrationConnections.id, connection.id), eq(integrationConnections.organizationId, context.organizationId), eq(integrationConnections.provider, MONERIS_PROVIDER), eq(integrationConnections.syncLeaseOwner, lease.owner), eq(integrationConnections.syncVersion, lease.version))).returning({ id: integrationConnections.id });
      ...
      return jsonResponse({ hasMore, retryRequired: warnings > 0, run: { id: runId, recordsRead, recordsStaged: staged, duplicatesSkipped: Math.max(0, recordsRead - staged - warnings), warningCount: warnings }, reconciliation: { payments: Number(totals?.count ?? 0), grossCents: Number(totals?.amountCents ?? 0), pages, daysProcessed }, readyForReview: Number(totals?.count ?? 0) > 0 && warnings === 0, nextStep: hasMore ? "Payment history is staged. Sync again to continue the resumable backfill." : "Payment history is staged. Compare its count and amount with Moneris before approval." });
    } catch (error) {
