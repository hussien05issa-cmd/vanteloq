...
export async function runSync(request: Request, requestId: string, context: SyncContext, input: Record<string, unknown>, trigger: SyncTrigger) {
    ...
    if (reason === "auto") return jsonResponse({ provider: CLOVER_PROVIDER, connectionId: connection.id, skipped: true, nextStep: "Clover background refresh is controlled by the automatic sync setting." });
    ...
    try {
      if (trigger === "manual") await enforceRateLimit("clover:manual-sync", context.organizationId, 30, 3600);
      await getD1().prepare(`
