...
export function buildExecutiveReport(period:ExecutivePeriod,sales:Sales,finance:Finance,financeReason:string|null,basis:"commerce"|"ledger") {
  ...
  const fromSales:Partial<Record<ExecutiveKey,string>>={net_revenue:"net_sales",gross_profit:"gross_profit",gross_margin:"gross_margin",inventory_value:"inventory_value",transactions:"transactions",average_order_value:"average_transaction",units_sold:"units"};
  const priorSales={net_revenue:sales.previous?.netSalesCents??null,gross_profit:sales.previous?.grossProfitCents??null,gross_margin:sales.previous?.grossMarginRate??null,inventory_value:sales.reportingPeriod?.previousInventoryValueCents??null,transactions:sales.previous?.transactionCount??null,average_order_value:sales.previous?.averageTransactionCents??null,units_sold:sales.previous?.unitsSold??null};
