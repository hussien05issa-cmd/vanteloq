...
export const managementConsoleAccess = sqliteTable(
  ...
  (table) => [
    uniqueIndex("management_console_access_email_unique").on(table.email),
    index("management_console_access_active_idx").on(table.active, table.role),
    check("management_console_access_role_check", sql`${table.role} in ('admin','viewer')`),
    check("management_console_access_email_check", sql`length(${table.email}) between 3 and 255 and instr(${table.email}, '@') > 1`),
  ],
);

export const managementContacts = sqliteTable(
  "management_contacts",
  {
    id: text("id").primaryKey(),
    company: text("company", { enum: ["lexedge", "vanteloq"] }).notNull(),
    name: text("name").notNull(),
    email: text("email").notNull().default(""),
    phone: text("phone").notNull().default(""),
    organization: text("organization").notNull().default(""),
    stage: text("stage", { enum: ["lead", "prospect", "client", "partner", "inactive"] }).notNull().default("lead"),
  ...
  (table) => [
    index("management_contacts_company_stage_idx").on(table.company, table.stage),
    index("management_contacts_follow_up_idx").on(table.company, table.nextFollowUpAt),
    check("management_contacts_company_check", sql`${table.company} in ('lexedge','vanteloq')`),
    check("management_contacts_stage_check", sql`${table.stage} in ('lead','prospect','client','partner','inactive')`),
    check("management_contacts_name_check", sql`length(${table.name}) between 1 and 160`),
  ],
);

export const managementActivities = sqliteTable(
  "management_activities",
  {
    id: text("id").primaryKey(),
    company: text("company", { enum: ["lexedge", "vanteloq"] }).notNull(),
    kind: text("kind", { enum: ["call", "meeting", "follow_up", "deadline"] }).notNull(),
    title: text("title").notNull(),
    contactId: text("contact_id").references(() => managementContacts.id, { onDelete: "set null" }),
  ...
  (table) => [
    index("management_activities_company_start_idx").on(table.company, table.startsAt),
    index("management_activities_status_idx").on(table.status, table.startsAt),
    check("management_activities_company_check", sql`${table.company} in ('lexedge','vanteloq')`),
    check("management_activities_kind_check", sql`${table.kind} in ('call','meeting','follow_up','deadline')`),
    check("management_activities_status_check", sql`${table.status} in ('scheduled','completed','cancelled','no_show')`),
    check("management_activities_title_check", sql`length(${table.title}) between 1 and 200`),
  ],
);

export const managementTasks = sqliteTable(
  "management_tasks",
  {
    id: text("id").primaryKey(),
    company: text("company", { enum: ["lexedge", "vanteloq"] }).notNull(),
  ...
  (table) => [
    index("management_tasks_company_status_idx").on(table.company, table.status),
    index("management_tasks_due_idx").on(table.status, table.dueAt),
    check("management_tasks_company_check", sql`${table.company} in ('lexedge','vanteloq')`),
    check("management_tasks_priority_check", sql`${table.priority} in ('high','medium','low')`),
    check("management_tasks_status_check", sql`${table.status} in ('open','in_progress','done')`),
    check("management_tasks_title_check", sql`length(${table.title}) between 1 and 200`),
  ],
);
...
export const marketingResourceSelections = sqliteTable(
  "marketing_resource_selections",
  {
    ...
    dataset: text("dataset", { enum: ["google_analytics", "google_search_console", "google_business_profile", "google_ads", "meta_ads"] }).notNull(),
  ...
  (table) => [
    ...
    check("marketing_resource_selections_dataset_check", sql`${table.dataset} in ('google_analytics','google_search_console','google_business_profile','google_ads','meta_ads')`),
    check("marketing_resource_selections_pair_check", sql`((${table.provider} = 'google' and ${table.dataset} in ('google_analytics','google_search_console','google_business_profile','google_ads')) or (${table.provider} = 'meta' and ${table.dataset} = 'meta_ads'))`),
...
export const financialTransactions = sqliteTable(
  "financial_transactions",
  {
    ...
    categoryAccountId: text("category_account_id").references(() => financialAccounts.id),
    contactId: text("contact_id").references(() => bookloqContacts.id),
    sourceSystem: text("source_system").notNull(),
  ...
  (table) => [
    ...
    index("financial_transactions_workspace_category_idx").on(table.organizationId, table.categoryAccountId, table.postingDate),
