...
export const tenantSubscriptions = sqliteTable(
  "tenant_subscriptions",
  {
    ...
    basePlan: text("base_plan", { enum: ["starter", "growth", "pro", "bookloq"] }),
  ...
  (table) => [
    ...
    check("tenant_subscriptions_plan_check", sql`${table.basePlan} is null or ${table.basePlan} in ('starter','growth','pro','bookloq')`),
    ...
    check("tenant_subscriptions_scheduled_plan_check", sql`${table.scheduledBasePlan} is null or ${table.scheduledBasePlan} in ('starter','growth','pro','bookloq')`),
...
export const integrationOAuthStates = sqliteTable(
  "integration_oauth_states",
  {
    ...
    provider: text("provider").notNull(),
    connectionId: text("connection_id").notNull().default("legacy"),
    initiatorAuthSubject: text("initiator_auth_subject"),
    initiatorAuthProvider: text("initiator_auth_provider", { enum: ["supabase", "sites"] }),
    initiatorAssuranceLevel: text("initiator_assurance_level", { enum: ["aal1", "aal2"] }),
    expiresAt: integer("expires_at", { mode: "timestamp" }).notNull(),
