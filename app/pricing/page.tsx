...
export default function PricingPage() {
 ...
 <section className="public-bookloq-price" aria-labelledby="bookloq-pricing-title"><div><p className="demo-eyebrow">COMMERCE-FIRST FINANCE WORKSPACE</p><h2 id="bookloq-pricing-title">Choose BookLoQ with Vanteloq or on its own.</h2><p>BookLoQ organizes source documents, invoices, transaction review and evidence matching, balanced journals, financial statements and 13-week cash review. It does not file returns, move money or certify that the books are complete.</p><div className="bookloq-price-links"><Link href="/demo#bookloq">Try the fictional cash planning demo →</Link><Link href="/resources/bookloq-vs-traditional-accounting-software">Compare BookLoQ workflows →</Link></div></div><div className="public-bookloq-price-options"><article><span>WITH A VANTELOQ PLAN</span><strong>+${ADDONS.bookloq.prices.month.amountCents/100}<small> CAD / month</small></strong><p>Shares the workspace, permissions and source infrastructure already included with Vanteloq.</p><Link className="bookloq-addon-action" href="#plan-starter">Choose a Vanteloq plan ↑</Link></article><article><span>BOOKLOQ STANDALONE</span><strong>${PLANS.bookloq.prices.month.amountCents/100}<small> CAD / month</small></strong><p>Includes its own secured workspace, source handling, permissions and BookLoQ access.</p><Link data-public-event="plan_selected" href={planSelectionUrl({plan:"bookloq",bookloq:false})}>Choose BookLoQ →</Link></article></div></section><CustomPlanCallout/>
 ...
 <div className="demo-conversion pricing-demo-conversion"><div><span className="pricing-demo-label">INTERACTIVE, FICTIONAL RECORDS</span><h2>See what your next decision could look like.</h2><p>Change a location or cash assumption, inspect the calculation and follow the result to its source rows.</p></div><Link href="/demo">Explore the demo →</Link></div>
