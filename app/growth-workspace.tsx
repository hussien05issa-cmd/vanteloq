  ...
  return <div className="content module-page growth-page">
    ...
    {tab === "data" && <section className="growth-data-layout">
      ...
      <form className="card growth-data-form" onSubmit={saveObservation}>
        ...
        <div className="growth-field-grid"><label className="wide"><FieldLabel>Search Query</FieldLabel><input name="query" maxLength={180} required placeholder="Search phrase exactly as shown in the export" /></label><label><FieldLabel>Observation Date</FieldLabel><input name="observedDate" type="date" required /></label><label><FieldLabel>Average Position</FieldLabel><input name="position" type="number" min="0.001" max="1000" step="0.001" required /></label></div>
    ...
    {tab === "calendar" && <section className="growth-calendar-layout">
      <form key={draftVersion} className="card growth-calendar-form" onSubmit={saveCalendar}>
        ...
        <div className="growth-field-grid"><label className="wide"><FieldLabel>Title</FieldLabel><input name="title" defaultValue={calendarDraft?.title ?? ""} required maxLength={180} placeholder="Publish service-area guide" /></label><label>Channel<select name="channel" defaultValue={calendarDraft?.channel ?? "content"}><option value="content">Content</option><option value="google">Google</option><option value="meta">Meta</option><option value="email">Email</option><option value="local">Local</option><option value="website">Website</option></select></label><label>Work type<select name="eventType" defaultValue={calendarDraft?.eventType ?? "content"}><option value="campaign">Campaign</option><option value="content">Content</option><option value="audit">Audit</option><option value="offer">Offer</option><option value="follow_up">Follow up</option></select></label><label><FieldLabel>Start Date</FieldLabel><input name="startDate" type="date" required /></label><label>Due date<input name="dueDate" type="date" /></label><label className="wide">Objective<textarea name="objective" defaultValue={calendarDraft?.objective ?? ""} maxLength={500} placeholder="What should change, and how will you measure it?" /></label><label className="wide">Notes<textarea name="notes" defaultValue={calendarDraft?.notes ?? ""} maxLength={1000} placeholder="Owner, dependencies, audience, offer details, or evidence needed" /></label></div>
