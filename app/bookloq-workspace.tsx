...
type CashPeriod = { startDate: string; endDate: string; transactionCount: number; inflowCents: number; outflowCents: number; netCashFlowCents: number; averageDailyOutflowCents: number; categorizedBasisPoints: number; matchedBasisPoints: number; categories: { name: string; amountCents: number; shareBasisPoints: number }[] };
...
function TransactionDrawer({ transaction, data, canEdit, close, saved }: { transaction: Transaction; data: BookLoQData; canEdit: boolean; close: () => void; saved: (message: string) => Promise<void> }) {
  const categories = data.statements.accounts.filter((account) => !["cash", "bank", "accounts_receivable", "accounts_payable"].includes(account.accountSubtype));
  ...
  const createCategory = async () => { if (customName.trim().length < 2) return; setSaving(true); const response = await apiFetch("/api/v1/bookloq/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "create_category", categoryName: customName, categoryType: transaction.amountCents >= 0 ? "revenue" : "expense" }) }); const body = await response.json(); if (!response.ok) { setError(body.error?.message ?? "Category could not be created."); setSaving(false); return; } setAccountId(body.category.id); setCustomName(""); setSaving(false); };
  const submit = async () => { if (!accountId) { setError("Choose a ledger category."); return; } setSaving(true); const response = await apiFetch("/api/v1/bookloq/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "categorize_transaction", transactionId: transaction.id, accountId, createRule, matchText: transaction.description, direction: transaction.amountCents >= 0 ? "inflow" : "outflow" }) }); const body = await response.json(); if (!response.ok) { setError(body.error?.message ?? "Category could not be saved."); setSaving(false); return; } await saved(createRule ? "Category confirmed and reusable merchant rule saved" : "Transaction category confirmed and recorded in the audit trail"); };
  const match = async (targetType: MatchCandidate["kind"], targetId: string) => { setSaving(true); const response = await apiFetch("/api/v1/bookloq/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "match_transaction", transactionId: transaction.id, targetType, targetId }) }); const body = await response.json(); if (!response.ok) { setError(body.error?.message ?? "Supporting record could not be matched."); setSaving(false); return; } await saved("Transaction matched to supporting evidence and added to the audit trail"); };
  return <div className="bookloq-record-drawer transaction-review"><div><span><small>TRANSACTION REVIEW</small><h3>{transaction.description}</h3></span><button aria-label="Close transaction" onClick={close}>×</button></div><dl><span><dt>Original description</dt><dd>{transaction.originalDescription}</dd></span><span><dt>Posting date</dt><dd>{shortDate(transaction.postingDate)}</dd></span><span><dt>Source</dt><dd>{label(transaction.sourceSystem)}</dd></span><span><dt>Amount</dt><dd>{money(transaction.amountCents, transaction.currency)}</dd></span><span><dt>Evidence</dt><dd>{existingMatch ? `Matched · ${existingMatch.targetLabel}` : label(transaction.reconciliationStatus)}</dd></span></dl><label><span>Business category</span><select value={accountId} onChange={(event) => setAccountId(event.target.value)} disabled={!canEdit}><option value="">Select a category</option>{categories.map((account) => <option key={account.id} value={account.id}>{account.code} · {account.name}</option>)}</select><small>This changes the accounting category, not the source bank account.</small></label><div className="custom-category-row"><input value={customName} maxLength={120} onChange={(event) => setCustomName(event.target.value)} placeholder={transaction.amountCents >= 0 ? "New income category" : "New expense category"}/><button disabled={!canEdit || saving || customName.trim().length < 2} onClick={() => void createCategory()}>Create category</button></div><label className="category-rule-check"><input type="checkbox" checked={createRule} onChange={(event) => setCreateRule(event.target.checked)}/><span>Suggest this category for future transactions containing “{transaction.description.slice(0, 48)}”</span></label>{!existingMatch && (suggestedMatches.length > 0 || receiptTargets.length > 0) && <section className="transaction-match-list"><small>MATCH SUPPORTING EVIDENCE</small>{suggestedMatches.map((candidate) => <button key={`${candidate.kind}:${candidate.candidateId}`} disabled={saving || !data.permissions.includes("reconcile_accounts")} onClick={() => void match(candidate.kind, candidate.candidateId)}><span><b>{candidate.label}</b><small>{candidate.reasons.join(" · ")}</small></span><strong>{Math.round(candidate.confidenceBasisPoints / 100)}%</strong></button>)}{receiptTargets.map((document) => <button key={document.id} disabled={saving || !data.permissions.includes("reconcile_accounts")} onClick={() => void match("receipt", document.id)}><span><b>{document.fileName}</b><small>Verified receipt · manual confirmation</small></span><strong>Review</strong></button>)}</section>}{error && <p className="bookloq-form-error">{error}</p>}<button className="bookloq-primary" disabled={!canEdit || saving || !accountId} onClick={() => void submit()}>{saving ? "Saving…" : "Confirm category"}</button></div>;
}
...
function cashTimeline(transactions: Transaction[], days: 30 | 90 | 365) {
  const today = new Date();
  const end = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate()));
  const start = new Date(end.getTime() - (days - 1) * 86_400_000);
  const bucketCount = days === 365 ? 12 : 6;
  const bucketDays = Math.ceil(days / bucketCount);
  const buckets = Array.from({ length: bucketCount }, (_, index) => {
    const from = new Date(start.getTime() + index * bucketDays * 86_400_000);
    const to = new Date(Math.min(end.getTime(), from.getTime() + (bucketDays - 1) * 86_400_000));
    return { from, to, inflowCents: 0, outflowCents: 0 };
  });
  for (const transaction of transactions) {
    const time = Date.parse(`${transaction.postingDate}T00:00:00Z`);
    if (!Number.isFinite(time) || time < start.getTime() || time > end.getTime()) continue;
    const index = Math.min(bucketCount - 1, Math.floor((time - start.getTime()) / (bucketDays * 86_400_000)));
    if (transaction.amountCents >= 0) buckets[index].inflowCents += transaction.amountCents;
    else buckets[index].outflowCents += Math.abs(transaction.amountCents);
  }
  return buckets.map((bucket) => ({
    ...bucket,
    label: days === 365
      ? new Intl.DateTimeFormat("en-CA", { month: "short", timeZone: "UTC" }).format(bucket.from)
      : new Intl.DateTimeFormat("en-CA", { month: "short", day: "numeric", timeZone: "UTC" }).format(bucket.from),
  }));
}

function BusinessCashReport({ data }: { data: BookLoQData }) {
  const [days, setDays] = useState<30 | 90 | 365>(90);
  const summary = days === 30 ? data.cashActivity.days30 : days === 90 ? data.cashActivity.days90 : data.cashActivity.months12;
  const timeline = useMemo(() => cashTimeline(data.transactions, days), [data.transactions, days]);
  const maximum = Math.max(1, ...timeline.flatMap((bucket) => [bucket.inflowCents, bucket.outflowCents]));
  ...
  return <section className="business-cash-report" aria-label="Business cash activity report">
    <header>
      <div><p>BUSINESS CASH ACTIVITY</p><h3>Actual inflows and outflows</h3><span>Posted financial activity only. Forecasts and commitments remain in the separate 13-week view below.</span></div>
      <div className="cash-period-switch" role="group" aria-label="Cash activity period"><button className={days === 30 ? "active" : ""} onClick={() => setDays(30)}>30 days</button><button className={days === 90 ? "active" : ""} onClick={() => setDays(90)}>90 days</button><button className={days === 365 ? "active" : ""} onClick={() => setDays(365)}>12 months</button></div>
    </header>
    <div className="business-cash-kpis"><article><span>Total inflows</span><b className="positive">{money(summary.inflowCents, data.organization.currency)}</b><small>{summary.transactionCount} reviewed and imported records</small></article><article><span>Total outflows</span><b className="negative">{money(summary.outflowCents, data.organization.currency)}</b><small>{money(summary.averageDailyOutflowCents, data.organization.currency)} average per day</small></article><article><span>Net cash flow</span><b className={summary.netCashFlowCents < 0 ? "negative" : "positive"}>{money(summary.netCashFlowCents, data.organization.currency)}</b><small>{shortDate(summary.startDate)}–{shortDate(summary.endDate)}</small></article><article><span>Review coverage</span><b>{rate(summary.categorizedBasisPoints)}</b><small>{coverage}</small></article></div>
    <div className="business-cash-body">
      <article className="cash-activity-chart"><div className="cash-activity-legend"><span><i className="in"/>Inflows</span><span><i className="out"/>Outflows</span></div><div className="cash-activity-bars">{timeline.map((bucket) => <div className="cash-activity-bucket" key={bucket.from.toISOString()} title={`${bucket.label}: inflows ${money(bucket.inflowCents, data.organization.currency)}, outflows ${money(bucket.outflowCents, data.organization.currency)}`}><div><i className="in" style={{ height: `${Math.max(bucket.inflowCents ? 4 : 0, bucket.inflowCents * 100 / maximum)}%` }}/><i className="out" style={{ height: `${Math.max(bucket.outflowCents ? 4 : 0, bucket.outflowCents * 100 / maximum)}%` }}/></div><span>{bucket.label}</span></div>)}</div>{!summary.transactionCount && <div className="cash-activity-empty">Connect a bank or import reviewed commerce activity to populate this report.</div>}</article>
