...
const sectionDefinitions = [
  { name: "Overview", label: "Overview", group: "Start", permission: "view_profit" },
  { name: "Transactions", label: "Work to Review", group: "Start", permission: "view_profit" },
  { name: "Banking", label: "Banking and Settlements", group: "Money Movement", permission: "view_banking" },
  { name: "Reconciliation", label: "Reconciliation", group: "Money Movement", permission: "reconcile_accounts" },
  { name: "Sales", label: "Sales", group: "Sales and Invoices", permission: "view_revenue" },
  { name: "Invoicing", label: "Invoicing", group: "Sales and Invoices", permission: "view_revenue" },
  { name: "Customers", label: "Customers", group: "Sales and Invoices", permission: "view_revenue" },
  { name: "Expenses", label: "Spend and Expenses", group: "Purchases and Documents", permission: "view_profit" },
  { name: "Bills", label: "Bills", group: "Purchases and Documents", permission: "view_profit" },
  { name: "Suppliers", label: "Suppliers", group: "Purchases and Documents", permission: "view_profit" },
  { name: "Inventory Accounting", label: "Inventory Finance", group: "Inventory Finance", permission: "view_profit" },
  { name: "Cash Flow", label: "Cash Plan", group: "Reports and Planning", permission: "view_banking" },
  { name: "Reports", label: "Reports", group: "Reports and Planning", permission: "view_profit" },
  { name: "Budgets", label: "Budgets", group: "Reports and Planning", permission: "view_profit" },
  { name: "Assets and Loans", label: "Assets and Loans", group: "Reports and Planning", permission: "view_profit" },
  { name: "Month-End", label: "Month-End", group: "Accountant Tools and Settings", permission: "reconcile_accounts" },
  { name: "Chart of Accounts", label: "Chart of Accounts", group: "Accountant Tools and Settings", permission: "view_profit" },
  { name: "Journal Entries", label: "Journal Entries", group: "Accountant Tools and Settings", permission: "post_journals" },
  { name: "Sales Tax", label: "Sales Tax", group: "Accountant Tools and Settings", permission: "view_profit" },
  { name: "Payroll", label: "Payroll", group: "Accountant Tools and Settings", permission: "view_payroll" },
  { name: "Accountant Portal", label: "Accountant Portal", group: "Accountant Tools and Settings", permission: "view_audit_logs" },
  { name: "Audit Trail", label: "Audit Trail", group: "Accountant Tools and Settings", permission: "view_audit_logs" },
  { name: "BookLoQ Assistant", label: "BookLoQ Assistant", group: "Accountant Tools and Settings", permission: "view_profit" },
  { name: "Settings", label: "Settings", group: "Accountant Tools and Settings", permission: "manage_integrations" },
] as const satisfies readonly { name: string; label: string; group: string; permission: Permission }[];

const sectionLabel = (name: Section) => sectionDefinitions.find(item => item.name === name)?.label ?? name;
...
export default function BookLoQWorkspace({ initialSection = "Overview", createTask, showNotice, navigate, activeLocationId }: { initialSection?: Section; createTask: (seed: TaskSeed) => void; showNotice: (message: string) => void; navigate: (view: "Integrations" | "Documents" | "Advisor") => void; activeLocationId: string | null }) {
  ...
  const visible = useMemo(() => authorizedSections.filter((item) => `${item.name} ${item.label} ${item.group}`.toLowerCase().includes(navSearch.toLowerCase())), [authorizedSections, navSearch]);
  const selectSection = (next: Section) => { if (authorizedSections.some(item => item.name === next)) setSection(next); else showNotice(`${next} requires additional finance access. Contact your workspace administrator.`); };
  ...
  return <section className={displayClass} aria-label="BookLoQ" aria-busy={loading}><div className={`bookloq-shell ${collapsed ? "bookloq-collapsed" : ""}`}>
    ...
    <main className="bookloq-main">
      ...
      <BookLoQSection section={activeSection} data={data} setSection={selectSection} createTask={createTask} showNotice={showNotice} refresh={refresh} openJournal={() => setJournalOpen(true)} navigate={navigate}/>
    </main>
...
function BookLoQBankConnection({ plaid, loading, error, canManage, onChanged, retry, showNotice }: { plaid: PlaidAccess | null; loading: boolean; error: string; canManage: boolean; onChanged: () => Promise<void>; retry: () => Promise<void>; showNotice: (message: string) => void }) {
  ...
  useModalFocus(approvalDialogRef, approvalOpen, () => { if (!approvalBusy) setApprovalOpen(false); });
...
function WorkReviewQueue({ data, createTask, openRecord }: { data: BookLoQData; createTask: (seed: TaskSeed) => void; openRecord: (item: BookloqReviewItem) => void }) {
  const asOf = data.thirteenWeekCashFlow.asOf;
  const queue = useMemo(() => buildBookloqReviewQueue({ alerts: data.alerts, transactions: data.transactions, bills: data.bills, closeItems: data.closeItems, documents: data.documents, asOf }), [data, asOf]);
  const [filter, setFilter] = useState<"all" | "high" | "money" | "close">("all");
  const filtered = queue.filter(item => filter === "all" || filter === "high" && item.priority === "high" || filter === "money" && ["transaction", "bill"].includes(item.kind) || filter === "close" && ["close", "document"].includes(item.kind));
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const selected = filtered.find(item => item.id === selectedId) ?? filtered[0] ?? null;
  const task = selected?.kind === "alert" ? data.alerts.find(item => item.id === selected.sourceRef) : null;
  const age = (days: number | null) => days === null ? "Age unavailable" : days === 0 ? "Recorded today" : `${days} day${days === 1 ? "" : "s"} open`;
  return <section className="bookloq-review-workspace" aria-labelledby="bookloq-review-title">
    <header><div><p>WORK TO REVIEW</p><h3 id="bookloq-review-title">Resolve the material exceptions first.</h3><span>Ranked from the records BookLoQ can verify. Missing amounts remain unmeasured.</span></div><div className="bookloq-review-summary" aria-label="Review queue summary"><span><b>{queue.filter(item => item.priority === "high").length}</b> High priority</span><span><b>{queue.length}</b> Open items</span></div></header>
    <div className="bookloq-review-filters" role="group" aria-label="Filter work to review">
      {([ ["all", "All"], ["high", "High priority"], ["money", "Money movement"], ["close", "Documents and close"] ] as const).map(([value, text]) => <button type="button" key={value} aria-pressed={filter === value} className={filter === value ? "active" : ""} onClick={() => { setFilter(value); setSelectedId(null); }}>{text}</button>)}
    </div>
    <span className="sr-only" role="status" aria-live="polite">{selected ? `${selected.title} selected` : "No review item selected"}</span>
    {selected ? <div className="bookloq-review-body"><div className="bookloq-review-list" aria-label="Materiality-ranked review items">{filtered.map(item => <button type="button" aria-pressed={selected.id === item.id} className={selected.id === item.id ? "selected" : ""} key={item.id} onClick={() => setSelectedId(item.id)}><span className={`review-priority ${item.priority}`}>{item.priority}</span><span><b>{item.title}</b><small>{item.reason}</small><em>{age(item.ageDays)} · {item.owner}</em></span><strong>{item.amountCents === null ? "Impact unmeasured" : money(item.amountCents, data.organization.currency)}</strong></button>)}</div><aside className="bookloq-review-detail"><span className={`review-priority ${selected.priority}`}>{selected.priority} priority</span><h4>{selected.title}</h4><p>{selected.reason}</p><dl><div><dt>Amount or impact</dt><dd>{selected.amountCents === null ? "Not measured from current records" : money(selected.amountCents, data.organization.currency)}</dd></div><div><dt>Source</dt><dd>{selected.source}</dd></div><div><dt>Age</dt><dd>{age(selected.ageDays)}</dd></div><div><dt>Owner</dt><dd>{selected.owner}</dd></div><div><dt>Confidence</dt><dd>{selected.confidence ? label(selected.confidence) : "Not scored"}</dd></div></dl><div className="bookloq-review-next"><small>NEXT ACTION</small><b>{selected.nextAction}</b></div><footer><button type="button" className="bookloq-primary" onClick={() => openRecord(selected)}>Open Supporting Records</button>{task && <button type="button" onClick={() => createTask(alertTask(task, data.organization.currency))}>Create Task</button>}</footer></aside></div> : <div className="bookloq-review-empty"><b>No items match this view.</b><span>This only means the current records did not produce an exception. It does not establish that the books are complete.</span></div>}
  </section>;
}

function TransactionCentre({ data, refresh, showNotice, setSection, createTask, navigate }: { data: BookLoQData; refresh: () => Promise<void>; showNotice: (message: string) => void; setSection: (section: Section) => void; createTask: (seed: TaskSeed) => void; navigate: (view: "Documents") => void }) {
  ...
  const openReviewRecord = (item: BookloqReviewItem) => {
    if (item.target === "Documents") { navigate("Documents"); return; }
    if (item.target !== "Transactions") { setSection(item.target); return; }
    const evidenceId = item.evidenceRef?.startsWith("transaction:") ? item.evidenceRef.slice("transaction:".length) : item.kind === "transaction" ? item.sourceRef : null;
    const supportingTransaction = evidenceId ? data.transactions.find(transaction => transaction.id === evidenceId) : null;
    if (supportingTransaction) { setSelected(supportingTransaction); return; }
    setPeriod("all");
    setReview("needs_review");
    setQuery(item.kind === "transaction" ? item.title : "");
    document.querySelector(".bookloq-table-card")?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  return <div className="bookloq-content"><PageIntro eyebrow="FINANCIAL REVIEW" title="Work to Review" copy="Start with the material exceptions, then confirm categories, evidence and matches without changing the original source record."/>
    <WorkReviewQueue data={data} createTask={createTask} openRecord={openReviewRecord}/>
    <section className="transaction-status-row" aria-label="Transaction review states"><button aria-pressed={review === "all"} className={review === "all" ? "active" : ""} onClick={() => setReview("all")}><b>{data.transactions.length}</b><span>All transactions</span></button><button aria-pressed={review === "needs_review"} className={review === "needs_review" ? "active" : ""} onClick={() => setReview("needs_review")}><b>{data.transactions.filter((item) => item.categorizationStatus !== "confirmed").length}</b><span>Needs review</span></button><button aria-pressed={review === "matched"} className={review === "matched" ? "active" : ""} onClick={() => setReview("matched")}><b>{matchedCount}</b><span>Matched to evidence</span></button><div className="transaction-stat"><b>{data.documentSummary.receipts}</b><span>Receipts available</span></div></section>
    <div className="bookloq-toolbar"><label><span className="sr-only">Search transactions</span><span aria-hidden="true">⌕</span><input aria-label="Search transactions" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search merchant, customer, category or source"/></label><select aria-label="Transaction period" value={period} onChange={(event) => setPeriod(event.target.value as typeof period)}><option value="30">Latest Recorded 30 Days</option><option value="90">Latest Recorded 90 Days</option><option value="365">Latest Recorded 12 Months</option><option value="all">All imported history</option></select><select aria-label="Transaction review state" value={review} onChange={(event) => setReview(event.target.value)}><option value="all">All review states</option><option value="needs_review">Needs category review</option><option value="matched">Matched to evidence</option><option value="confirmed">Category confirmed</option><option value="suggested">Rule suggestion</option><option value="unreconciled">Unreconciled</option></select>{data.permissions.includes("export_data") && <button onClick={() => downloadTransactions(filtered)}>Export CSV</button>}</div>
    <p className="calculation-note">{data.transactions.length ? `${period === "all" ? "All imported history" : shortDate(start)} through ${shortDate(latestPostingDate)}. Filters use the latest recorded transaction date.` : "No transaction dates available yet."}</p><article className="bookloq-card bookloq-table-card"><div className="bookloq-table" role="region" aria-label="Transaction records" tabIndex={0}><div className="bookloq-tr bookloq-th"><span>Date</span><span>Description</span><span>Source</span><span>Account</span><span>Review state</span><span>Amount</span></div>{filtered.map((transaction) => <button className="bookloq-tr" key={transaction.id} onClick={() => setSelected(transaction)}><span>{shortDate(transaction.postingDate)}</span><span><b>{transaction.description}</b><small>{transaction.contactName ?? transaction.originalDescription}</small></span><span>{label(transaction.sourceSystem)}</span><span>{transaction.accountCode ? `${transaction.accountCode} · ${transaction.accountName}` : "Uncategorized"}</span><span><i className={`state ${transaction.categorizationStatus}`}>{label(transaction.categorizationStatus)}</i><small>{Math.round(transaction.confidenceBasisPoints / 100)}% confidence</small></span><span className={transaction.amountCents < 0 ? "negative" : "positive"}>{money(transaction.amountCents, transaction.currency)}</span></button>)}</div>{!filtered.length && <EmptyLine text="No transaction matches this view."/>}</article>
...
function TransactionDrawer({ transaction, data, canEdit, close, saved }: { transaction: Transaction; data: BookLoQData; canEdit: boolean; close: () => void; saved: (message: string) => Promise<void> }) {
  ...
  useModalFocus(drawerRef, true, () => { if (!saving) close(); });
...
function ReceivablesPanel({ data, refresh, showNotice }: { data: BookLoQData; refresh: () => Promise<void>; showNotice: (message: string) => void }) {
  ...
  const [emailing, setEmailing] = useState<InvoiceEmailSeed | null>(null);
  ...
  return <div className="bookloq-content"><PageIntro eyebrow="SALES AND ACCOUNTS RECEIVABLE" title="Create, send and track professional invoices" copy="Build a branded invoice, save its PDF to Files and email the exact saved document to your customer." action={canCreate ? <button className="bookloq-primary" onClick={() => setCreating(true)}>+ New invoice</button> : undefined}/><section className="bookloq-kpis compact"><FinancialKpi label="Posted revenue" value={money(data.summary.revenueCents, data.organization.currency)} note="Revenue accounts"/><FinancialKpi label="Outstanding A/R" value={money(data.summary.accountsReceivableCents, data.organization.currency)} note="Posted receivable balance"/><FinancialKpi label="Overdue" value={bookloqMetricCount(data.summary.overdueInvoicesCount)} note="Issued invoices with an unpaid balance"/></section><DataTable headings={["Invoice", "Customer", "Issued", "Due", "State", "Outstanding", "Actions"]} rows={data.invoices.map((invoice) => [invoice.invoiceNumber, invoice.customerName, shortDate(invoice.invoiceDate), shortDate(invoice.dueDate), label(invoice.status), money(invoice.totalCents - invoice.paidCents, invoice.currency), invoice.documentId ? <div className="invoice-actions"><button type="button" className="table-action invoice-download" disabled={downloadingDocumentId === invoice.documentId} onClick={() => void downloadInvoice(invoice)}>{downloadingDocumentId === invoice.documentId ? "Downloading…" : "Download PDF"}</button>{canCreate && !["void", "written_off"].includes(invoice.status) && <button type="button" className="table-action" onClick={() => setEmailing({ id: invoice.id, invoiceNumber: invoice.invoiceNumber, customerEmail: invoice.emailedTo || invoice.customerEmail, message: "" })}>Email Invoice</button>}</div> : "Not generated"])}/>{!canCreate && <ProviderGate title="Invoice creation requires finance access" detail="An owner or manager can grant draft-editing permission. Existing invoices remain available according to your role."/>}{creating && <InvoiceComposer data={data} close={() => setCreating(false)} saved={async (message, email) => { setCreating(false); if (email) setEmailing(email); showNotice(message); await refresh(); }}/ >}{emailing && <InvoiceEmailDialog invoice={emailing} close={() => setEmailing(null)} sent={async () => { setEmailing(null); showNotice("Invoice email accepted for delivery."); await refresh(); }}/>}</div>;
}


type InvoiceEmailSeed = { id: string; invoiceNumber: string; customerEmail: string; message: string };
function InvoiceEmailDialog({ invoice, close, sent }: { invoice: InvoiceEmailSeed; close: () => void; sent: () => Promise<void> }) {
  ...
  useModalFocus(ref, true, () => { if (!busy) close(); });
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError("");
    try {
      await bookloqRequest(apiFetch, "/api/v1/bookloq/invoices/email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ invoiceId: invoice.id, to: recipient, message }) }, 20_000);
      await sent();
    } catch (caught) { setError("The invoice is saved. Email delivery could not be confirmed. " + (caught instanceof Error ? caught.message : "Try again.")); }
    finally { setBusy(false); }
  };
  return createPortal(<div ref={ref} className="bookloq-drawer" role="dialog" aria-modal="true" aria-labelledby="invoice-email-title"><form className="invoice-composer invoice-email-composer" onSubmit={submit}><header><div><small>BOOKLOQ</small><h3 id="invoice-email-title">Email {invoice.invoiceNumber}</h3><p>Review the recipient and send the saved PDF. Retrying uses the same invoice.</p></div><button type="button" onClick={close} disabled={busy} aria-label="Close invoice email">×</button></header><label><FieldLabel>Recipient Email</FieldLabel><input type="email" required maxLength={254} value={recipient} disabled={busy} onChange={event => setRecipient(event.target.value)}/></label><label><FieldLabel required={false}>Message</FieldLabel><textarea maxLength={500} value={message} disabled={busy} onChange={event => setMessage(event.target.value)}/></label>{error && <p className="bookloq-form-error" role="alert">{error}</p>}<footer><button type="button" disabled={busy} onClick={close}>Close</button><button className="bookloq-primary" disabled={busy} type="submit">{busy ? "Sending…" : error ? "Retry Email" : "Send Invoice"}</button></footer></form></div>, document.body);
}
...
function InvoiceComposer({ data, close, saved }: { data: BookLoQData; close: () => void; saved: (message: string, email?: InvoiceEmailSeed) => Promise<void> }) {
  ...
  const [lines, setLines] = useState<InvoiceLineDraft[]>([{ id: crypto.randomUUID(), description: "", quantity: "1", price: "", tax: "0" }]);
  ...
  const normalizedLines = () => lines.map(line => ({
    description: line.description, quantityMilli: decimalUnits(line.quantity || "0", 3),
    unitPriceCents: decimalUnits(line.price || "0", 2), taxRateBasisPoints: decimalUnits(line.tax || "0", 2),
  }));
  let totals: ReturnType<typeof invoiceTotals> | null = null;
  try { totals = invoiceTotals(normalizedLines().map(line => invoiceLineAmounts(line.quantityMilli, line.unitPriceCents, line.taxRateBasisPoints))); } catch { /* Invalid edits stay visible until corrected. */ }
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await saved(`Invoice ${fields.invoiceNumber} saved as a PDF in Files.`,
        requestedDelivery === "email" ? { id: body.invoice.id, invoiceNumber: fields.invoiceNumber, customerEmail: fields.customerEmail, message: fields.emailMessage } : undefined);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "The invoice could not be created."); }
    finally { setBusy(false); }
  };
  return createPortal(<div ref={modalRef} className="bookloq-drawer" role="dialog" aria-modal="true" aria-labelledby="invoice-composer-title"><form className="invoice-composer" onSubmit={submit}><header><div><small>BOOKLOQ INVOICE STUDIO</small><h3 id="invoice-composer-title">Create a customer invoice</h3><p>The PDF saved in Files is the same document attached to the email.</p></div><button type="button" aria-label="Close invoice creator" disabled={busy} onClick={close}>×</button></header><div className="invoice-form-grid"><section><h4>Your business</h4><p className="calculation-note">Your business name appears on the PDF. Custom logos are temporarily unavailable.</p><label><FieldLabel>Business or Legal Name</FieldLabel><input required maxLength={180} value={fields.issuerName} onChange={(event) => update("issuerName", event.target.value)}/></label><label><FieldLabel>Address</FieldLabel><textarea required maxLength={500} value={fields.issuerAddress} onChange={(event) => update("issuerAddress", event.target.value)}/></label><div className="invoice-inline"><label><FieldLabel required={false}>Email</FieldLabel><input type="email" maxLength={254} value={fields.issuerEmail} onChange={(event) => update("issuerEmail", event.target.value)}/></label><label><FieldLabel required={false}>Phone</FieldLabel><input maxLength={60} value={fields.issuerPhone} onChange={(event) => update("issuerPhone", event.target.value)}/></label></div><label><FieldLabel required={false}>Tax or Registration Number</FieldLabel><input maxLength={80} value={fields.issuerTaxNumber} onChange={(event) => update("issuerTaxNumber", event.target.value)}/></label></section><section><h4>Invoice details</h4><div className="invoice-inline"><label><FieldLabel>Invoice Number</FieldLabel><input required maxLength={80} value={fields.invoiceNumber} onChange={(event) => update("invoiceNumber", event.target.value)}/></label><label><FieldLabel>Currency</FieldLabel><input required pattern="[A-Za-z]{3}" maxLength={3} value={fields.currency} onChange={(event) => update("currency", event.target.value.toUpperCase())}/></label></div><div className="invoice-inline"><label><FieldLabel>Invoice Date</FieldLabel><input required type="date" value={fields.invoiceDate} onChange={(event) => update("invoiceDate", event.target.value)}/></label><label><FieldLabel>Due Date</FieldLabel><input required type="date" min={fields.invoiceDate} value={fields.dueDate} onChange={(event) => update("dueDate", event.target.value)}/></label></div><label><FieldLabel required={false}>Purchase Order or Reference</FieldLabel><input maxLength={120} value={fields.purchaseOrderRef} onChange={(event) => update("purchaseOrderRef", event.target.value)}/></label><h4>Bill to</h4>{customerRecords.length > 0 && <label>Use an existing customer <select value={fields.customerId} onChange={(event) => chooseCustomer(event.target.value)}><option value="">New customer</option>{customerRecords.map((customer) => <option value={customer.id} key={customer.id}>{customer.name}</option>)}</select></label>}<label><FieldLabel>Customer Name</FieldLabel><input required maxLength={180} value={fields.customerName} onChange={(event) => update("customerName", event.target.value)}/></label><label><FieldLabel>Billing Address</FieldLabel><textarea required maxLength={500} value={fields.customerAddress} onChange={(event) => update("customerAddress", event.target.value)}/></label><div className="invoice-inline"><label><FieldLabel required={delivery === "email"}>Customer Email</FieldLabel><input required={delivery === "email"} type="email" maxLength={254} value={fields.customerEmail} onChange={(event) => update("customerEmail", event.target.value)}/></label><label><FieldLabel required={false}>Phone</FieldLabel><input maxLength={60} value={fields.customerPhone} onChange={(event) => update("customerPhone", event.target.value)}/></label></div></section></div><section className="invoice-lines"><p className="calculation-note">Set the tax rate that applies to each item. Tax starts at 0% and is not selected automatically.</p><div><h4>Line items</h4><button type="button" disabled={lines.length >= 100} onClick={() => setLines((current) => [...current, { id: crypto.randomUUID(), description: "", quantity: "1", price: "", tax: "0" }])}>+ Add line</button></div><div className="invoice-line-head"><span>Description</span><span>Quantity</span><span>Unit price</span><span>Tax %</span><span/></div>{lines.map((line, index) => <div className="invoice-line-row" key={line.id}><label><span className="invoice-line-label">Description</span><input aria-label={`Description for line ${index + 1}`} required maxLength={500} value={line.description} onChange={(event) => setLines((current) => current.map((item) => item.id === line.id ? { ...item, description: event.target.value } : item))}/></label><label><span className="invoice-line-label">Quantity</span><input aria-label={`Quantity for line ${index + 1}`} required type="number" min="0.001" step="0.001" value={line.quantity} onChange={(event) => setLines((current) => current.map((item) => item.id === line.id ? { ...item, quantity: event.target.value } : item))}/></label><label><span className="invoice-line-label">Unit price</span><input aria-label={`Unit price for line ${index + 1}`} required type="number" min="0" step="0.01" value={line.price} onChange={(event) => setLines((current) => current.map((item) => item.id === line.id ? { ...item, price: event.target.value } : item))}/></label><label><span className="invoice-line-label">Tax %</span><input aria-label={`Tax rate for line ${index + 1}`} required type="number" min="0" max="100" step="0.01" value={line.tax} onChange={(event) => setLines((current) => current.map((item) => item.id === line.id ? { ...item, tax: event.target.value } : item))}/></label><button type="button" aria-label={`Remove line ${index + 1}`} disabled={lines.length === 1} onClick={() => setLines((current) => current.filter((item) => item.id !== line.id))}>×</button></div>)}</section><div className="invoice-bottom"><section><label><FieldLabel required={false}>Payment Instructions</FieldLabel><textarea maxLength={2000} value={fields.paymentInstructions} onChange={(event) => update("paymentInstructions", event.target.value)} placeholder="How and where the customer should pay"/></label><label><FieldLabel required={false}>Notes</FieldLabel><textarea maxLength={2000} value={fields.notes} onChange={(event) => update("notes", event.target.value)} placeholder="Terms, project details or a thank-you note"/></label>{<label><FieldLabel required={false}>Email Message</FieldLabel><textarea maxLength={500} value={fields.emailMessage} onChange={(event) => update("emailMessage", event.target.value)} placeholder="Included only when you email this invoice"/></label>}</section><aside><span>Subtotal <b>{totals ? money(totals.subtotalCents, displayCurrency) : "Check line items"}</b></span><span>Tax <b>{totals ? money(totals.taxCents, displayCurrency) : "Not available"}</b></span><strong>Amount due <b>{totals ? money(totals.totalCents, displayCurrency) : "Not available"}</b></strong></aside></div>{error && <p className="bookloq-form-error" role="alert">{error}</p>}<footer><button type="button" onClick={close}>Cancel</button><button type="submit" disabled={busy} onClick={() => setDelivery("save")}>{busy && delivery === "save" ? "Saving…" : "Save PDF to Files"}</button><button className="bookloq-primary" type="submit" disabled={busy} onClick={() => setDelivery("email")}>{busy && delivery === "email" ? "Saving…" : "Save and Review Email"}</button></footer></form></div>, document.body);
}
