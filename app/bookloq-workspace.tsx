...
export default function BookLoQWorkspace({ initialSection = "Overview", createTask, showNotice, navigate, activeLocationId }: { initialSection?: Section; createTask: (seed: TaskSeed) => void; showNotice: (message: string) => void; navigate: (view: "Integrations" | "Documents") => void; activeLocationId: string | null }) {
  ...
  return <div className={`bookloq-shell ${collapsed ? "bookloq-collapsed" : ""}`}>
    ...
    <main className="bookloq-main">
      <header className="bookloq-top"><div><p>VANTELOQ / BOOKLOQ</p><h2>{activeSection}</h2><label className="bookloq-mobile-nav"><span>BookLoQ workspace</span><select value={activeSection} onChange={(event) => setSection(event.target.value as Section)}>{[...new Set(authorizedSections.map((item) => item.group))].map((group) => <optgroup label={group} key={group}>{authorizedSections.filter((item) => item.group === group).map((item) => <option key={item.name} value={item.name}>{item.name}</option>)}</optgroup>)}</select></label></div><div>{health && <span className="bookloq-health"><i style={{ "--health": `${health.degrees}deg` } as React.CSSProperties}/><b>{health.score}</b><small>BOOKS HEALTH</small></span>}{data.permissions.includes("post_journals") && <button className="bookloq-primary" disabled={(data.accountCatalog ?? data.statements.accounts).length < 2} title={(data.accountCatalog ?? data.statements.accounts).length < 2 ? "Configure the chart of accounts and an open accounting period first." : undefined} onClick={() => setJournalOpen(true)}>+ Journal entry</button>}</div></header>
...
function JournalComposer({ data, close, saved }: { data: BookLoQData; close: () => void; saved: () => Promise<void> }) {
  ...
  return <div className="modal-backdrop" ref={modalRef} role="dialog" aria-modal="true" aria-label="Create balanced journal" tabIndex={-1}>
    <form className="bookloq-journal-form" onSubmit={submit}>
      ...
      <label>Entry date<input type="date" value={date} disabled={frozen} onChange={(event) => { setDate(event.target.value); setConfirmed(false); }} required/></label>
      <label>Source report reference and explanation<textarea value={memo} disabled={frozen} onChange={(event) => { setMemo(event.target.value); setConfirmed(false); }} maxLength={1000} required placeholder="Use aggregate totals and a nonidentifying source report reference."/></label>
