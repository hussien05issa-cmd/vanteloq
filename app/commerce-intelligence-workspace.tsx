...
type Snapshot = {
  ...
  inventory: Array<{
    provider: string; locationRef: string; sku: string; name: string; categoryRef: string | null; supplierRef: string | null;
    ...
    periodDiscountCents: number; defaultCostCents: number | null; updatedAt: number | string;
  }>;
  customers: Array<{
  ...
  permissions: { customerIdentity: boolean; productCosts: boolean; profit: boolean; suppliers: boolean; inventory: boolean };
};
...
function downloadInventoryCsv(rows: Snapshot["inventory"], from: string, to: string) {
  const header = ["SKU", "Item", "Location", "On hand", "Reorder point", "Units sold", "Daily velocity", "Days of cover", "Stock status", "Recommended order", "Period sales"];
  const body = rows.map((row) => [row.sku, row.name, row.locationRef, row.onHandQuantity, row.reorderPoint, row.unitsSold.toFixed(2), row.dailyVelocity.toFixed(2), row.daysOfCover?.toFixed(1) ?? "", row.stockStatus, row.recommendedOrderUnits, (row.periodNetSalesCents / 100).toFixed(2)]);
      ...
      <section className="dense-panel commerce-intelligence-table">
        ...
        {hasRecords ? <div className="commerce-scroll-table" role="region" aria-label={`${mode} records`} tabIndex={0}>
          ...
          {mode === "Inventory" && <div className="commerce-ledger commerce-ledger-inventory">
            <div className="commerce-ledger-head"><span>Item</span><span>Location</span><span>On hand</span><span>Velocity</span><span>Cover</span><span>Status</span><span>Order review</span></div>
            {inventory.map((row) => <div key={`${row.provider}:${row.locationRef}:${row.sku}`}><span><b>{row.name}</b><small>{row.sku}</small></span><span>{row.locationRef.split(":").at(-1)}</span><span>{row.onHandQuantity.toLocaleString()}</span><span>{row.dailyVelocity.toFixed(2)}/day</span><span>{row.daysOfCover == null ? "No velocity" : `${row.daysOfCover.toFixed(1)} days`}</span><span><i className={`stock-state ${row.stockStatus}`}>{row.stockStatus}</i></span><span><b>{row.recommendedOrderUnits > 0 ? `${row.recommendedOrderUnits} units` : "No order"}</b><small>Review—not automatic</small></span></div>)}
          </div>}
