...
export default function AdvisorComposer({ question, onQuestion, dataUseAccepted, onConsent, loading, thinking = loading, onSubmit, onNewChat, children, hasConversation = false, memoryEnabled = false, onMemory, privacyControls, provider = "gemini", onProvider, providers = { gemini: { ready: true }, openai: { ready: false } } }: Props) {
    ...
    <div className="ai-chat-column">
      ...
      <div className="ai-compose-area">
        <form onSubmit={event => { if (!ready) { event.preventDefault(); return; } setSuggestionsOpen(false); onSubmit(event); }}>
          ...
          }} maxLength={800} required aria-describedby="advisor-submit-help" placeholder="Ask about your business…"/>
        ...
        <div className="ai-consent-row"><label className="ai-consent"><input type="checkbox" checked={dataUseAccepted} disabled={loading} onChange={event => onConsent(event.target.checked)}/><span>I agree to send my question and permitted business data to <strong>{ADVISOR_PROVIDER_LABELS[provider]}</strong>.</span></label><button className="ai-text-button" type="button" onClick={() => openSettings(true)}>Data use</button></div>
