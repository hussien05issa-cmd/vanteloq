...
export default function VanteloqAiLogo({ size = 48, className = "", decorative = false, thinking = false, active = false }: { size?: number; className?: string; decorative?: boolean; thinking?: boolean; active?: boolean }) {
  ...
  return <span className={"vanteloq-ai-logo " + (size < 40 ? "is-compact " : "") + (thinking ? "is-thinking " : active ? "is-attentive " : "") + className} style={{ width: size, height: size }} role={decorative ? undefined : "img"} aria-label={decorative ? undefined : "Vanteloq AI logo"} aria-hidden={decorative || undefined}>
    <svg viewBox="0 0 160 160" fill="none" aria-hidden="true" focusable="false">
      <defs>
        <linearGradient id={id + "-left"} x1="33" y1="32" x2="101" y2="139" gradientUnits="userSpaceOnUse"><stop stopColor="#90D7FF"/><stop offset=".16" stopColor="#1F5FB6"/><stop offset=".49" stopColor="#102C62"/><stop offset=".86" stopColor="#285DAE"/><stop offset="1" stopColor="#62ADFF"/></linearGradient>
        <linearGradient id={id + "-right"} x1="116" y1="28" x2="89" y2="125" gradientUnits="userSpaceOnUse"><stop stopColor="#D9F5FF"/><stop offset=".17" stopColor="#70C8FF"/><stop offset=".46" stopColor="#0873F6"/><stop offset=".76" stopColor="#3E63EF"/><stop offset="1" stopColor="#BDA0FF"/></linearGradient>
        ...
        <pattern id={id + "-grid"} width="7" height="7" patternUnits="userSpaceOnUse"><path d="M7 0H0V7" stroke="#B4DFFF" strokeWidth=".35"/><circle cx=".7" cy=".7" r=".6" fill="#E4F5FF"/></pattern>
        <filter id={id + "-glow"} x="-80%" y="-80%" width="260%" height="260%"><feGaussianBlur stdDeviation="1.7"/></filter>
      </defs>
      <g className="vai-core">
        ...
        <path className="vai-grid" d="M110 35H136L101 112L88 83Z" fill={paint("grid")} opacity=".45"/>
