...
export default function SubprocessorsPage() {
  return <LegalShell updated="September 15, 2026" eyebrow="SERVICE PROVIDER NOTICE" title="Know where Vanteloq sends information." summary="This list separates core infrastructure from services a workspace chooses to connect.">
    ...
    <PolicySection id="optional" title="Optional customer directed services">
      ...
      <ul>
        <li><strong>Cloudflare Email Routing and Email Workers:</strong> optional document forwarding after workspace authorization and storage consent. Processes incoming email to send supported attachments to Vanteloq&apos;s existing private quarantine. Vanteloq retains originals, limited receipt metadata and an unverified sender address under its document retention controls, without keeping the email body. Cloudflare states that Email Routing does not store email content; delivery and security metadata and mailbox copies follow their own retention rules. This feature does not automatically scan documents, send them to AI or post accounting entries.</li>
        <li><strong>Microsoft Azure Blob Storage and Defender for Storage:</strong> temporary document security scanning when configured and an authorized user chooses Scan and Read. Receives the selected file under a random identifier in a private container. The application checks the file identity and a completed malware scan before releasing the original. Scanning copies are deleted after a confirmed result; interrupted copies remain subject to the scanning storage expiry policy. Azure storage recovery can retain deleted copies for 7 days. Scanning takes place in the configured storage region. Customer storage credentials and original filenames are not sent with the file.</li>
        ...
        <li><strong>OpenAI:</strong> optional Vanteloq AI analysis using the question, permission-filtered business and financial KPIs and bounded conversation context, plus user-selected file or photo contents when separately confirmed for that message, only when configured and selected with affirmative acceptance. Processing may occur in the United States and other provider locations. Requests disable stored response objects; provider safety retention can still apply.</li>
