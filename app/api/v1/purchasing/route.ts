    ...
    if (action === "create") {
      ...
      const lines = input.lines.map((entry, index) => {
        ...
        return {
          ...
          lineNumber: index + 1,
          sku: text(row.sku, "SKU", 80, false),
          description: text(row.description, "product description", 200),
          quantity,
          unitCostCents,
          previousCostCents:
            row.previousCostCents === null ||
            row.previousCostCents === undefined
              ? null
              : integer(row.previousCostCents, "previous cost"),
          currentInventory:
            row.currentInventory === null || row.currentInventory === undefined
              ? null
              : integer(row.currentInventory, "current inventory"),
          reorderPoint:
            row.reorderPoint === null || row.reorderPoint === undefined
              ? null
              : integer(row.reorderPoint, "reorder point"),
          forecastDemand:
            row.forecastDemand === null || row.forecastDemand === undefined
              ? null
              : integer(row.forecastDemand, "forecast demand"),
        };
      });
      ...
      details = { action, status, lineCount: lines.length, totalCents };
