...
import { fetchLightspeedRCollection, LIGHTSPEED_R_PROVIDER, normalizeLightspeedRSale } from "../../../../../../server/integrations/lightspeed-r";
...
export async function POST(request: Request) {
  return handleApi(request, async ({ requestId }) => {
    ...
    await enforceRateLimit("lightspeed-r:sample-sync", context.userId, 12, 3600);
    ...
    if (!connection?.externalAccountRef) throw new ApiError(409, "LIGHTSPEED_R_NOT_CONNECTED", "Authorize and verify R-Series before staging a sample.");
    ...
    await getDb().insert(integrationSyncRuns).values({
      ...
      mode: "sample", status: "running", cursorBefore: connection.lastSyncCursor, cursorAfter: null,
    ...
    try {
      const page = await fetchLightspeedRCollection(context.organizationId, connection.externalAccountRef, "Sale", { maxPages: 3 });
      const normalized = [];
      let warnings = 0;
      for (const sale of page.data) {
        try { normalized.push(await normalizeLightspeedRSale(sale)); } catch { warnings += 1; }
      }
      let staged = 0;
      ...
      await getDb().update(integrationSyncRuns).set({
        status: "completed", cursorAfter: safeCursor, recordsRead: page.data.length,
        recordsStaged: staged, duplicatesSkipped, warningCount: warnings, completedAt,
      }).where(eq(integrationSyncRuns.id, runId));
      await getDb().update(integrationConnections).set({
        lastSuccessfulSyncAt: completedAt, lastSyncCursor: safeCursor, dataPromotionStatus: "staging",
        lastErrorCode: null, updatedAt: completedAt,
      ...
      await recordAudit({
        ...
        details: { provider: LIGHTSPEED_R_PROVIDER, recordsRead: page.data.length, recordsStaged: staged, duplicatesSkipped, warningCount: warnings, dataPromotionEnabled: false },
      });
      return jsonResponse({
