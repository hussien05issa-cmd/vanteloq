...
| POST | `/api/v1/daily-metrics` | Active owner/admin/manager membership | 512 KiB | 12/hour/user | Same-origin and UUID `Idempotency-Key`; 1 to 366 unique date/location rows; convergent upserts; audited import |
...
| POST | `/api/v1/bookloq/journals` | Active owner/admin with `post_journals` | 64 KiB | 30/hour/user | Same-origin and UUID `Idempotency-Key`; integer minor units; 2 to 50 lines; active tenant accounts; balanced; open period; audited |
