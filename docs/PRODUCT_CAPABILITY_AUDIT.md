...
| Financial command centre | Gross profit, margin, contribution after labour, latest imported balances, scenario planner | Not a P&L or cash forecast until expenses, bills, payroll timing, tax, debt, and bank feeds exist |
| Bookkeeping | Workspace and required-source/action model | Transaction categorization, receipts, invoices, GST/HST, close workflow, accountant export, and reconciliation are not implemented |
