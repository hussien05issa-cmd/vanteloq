...
- R-Series net line revenue subtracts both `calcLineDiscount` and `calcTransactionDiscount` from the pre-discount `calcSubtotal`. A total-only fallback removes `calcTax1` and `calcTax2`. Unit FIFO cost is multiplied by signed units when aggregate costs are absent. Checkpoint version 5 restarts bounded historical imports to repair older normalized records and refresh catalogue coverage. See the [official SaleLine definition](https://developers.lightspeedhq.com/retail/endpoints/SaleLine/).
- Catalogue requests use the documented `ItemPrices` relation with `ItemShops`; `Prices` is a response field, not a relation name. Sync reconciliation remains visible during background refreshes and distinguishes approved data from an unfinished historical import.
