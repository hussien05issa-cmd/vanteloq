...
|---|---|
| Lightspeed X-Series | The read-only staging adapter is built. Add an approved app's client ID and secret to hosted secrets, preserve the exact callback and `2026-07` API version, authorize an owner account, map outlets, reconcile a bounded sales sample, fault-test refresh/rate-limit/replay paths, and complete a read-only canary before data promotion can be approved. See `LIGHTSPEED_X_SERIES.md`. |
