...
| Opportunities | Operating decision rules, retail discount/stock findings, marketing recommendations and action creation | Partial. The existing operating queue is now visible and inspectable. Unified inventory/customer/marketing opportunity aggregation, persistent dismiss/snooze, lifecycle history and deduplication across refreshes require an additive domain model. Do not claim these states already exist. |
...
| Operations | Tasks, assignee text, due dates, status, source reference, expected impact, Decision Journal before/after review | Partial, improved here. Structured member assignment, notes/activity history, reminders and direct action-to-outcome linking remain. Before/after observations do not establish intervention causality. |
...
2. Extend the existing decisions into a persistent opportunity lifecycle with organization/location permissions, evidence snapshots, dismissal/snooze and links to tasks. Avoid duplicate opportunities on refresh.
