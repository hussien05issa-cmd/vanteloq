...
8. **BookLoQ guided checks.** The existing assistant panel now links to real Vanteloq AI and clearly labels its local preset calculations. Missing GST records and empty month-end checklists stay unavailable; bank comparisons preserve their direction and do not invent settlement causes. The displayed calculation updates with current records instead of retaining an old answer. Numeric bank-sync timestamps are preserved in AI evidence.
