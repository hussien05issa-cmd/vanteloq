...
const bundle = await build({stdin:{contents:`import React, {useState} from 'react'; import AuthPanel from './app/auth-panel'; import SecureOnboardingFlow from './app/secure-onboarding-flow'; import {createRoot} from 'react-dom/client'; import {LandingPage} from './app/page'; import DemoPage from './app/demo/page'; import HelpPage from './app/help/page'; import PricingPage from './app/pricing/page'; import CustomPlanPage from './app/custom-plan/page'; import ContactPage from './app/contact/page'; function Preview(){const [mode,setMode]=useState(location.pathname==='/signup'?'signup':location.pathname==='/signin'?'signin':null); return location.pathname==='/setup'?<SecureOnboardingFlow accountName='Avery Chen' accountEmail='avery@example.com' complete={()=>{}} signOut={()=>{}}/>:<>{mode&&<AuthPanel initialMode={mode} close={()=>setMode(null)} authenticated={()=>{}}/>}{location.pathname === '/demo' ? <DemoPage/> : location.pathname === '/help' ? <HelpPage/> : location.pathname === '/pricing' ? <PricingPage/> : location.pathname === '/custom-plan' ? <CustomPlanPage/> : location.pathname === '/contact' ? <ContactPage/> : <LandingPage start={setMode}/>}</>;} createRoot(document.getElementById('root')).render(<Preview/>);`,resolveDir:process.cwd(),loader:'tsx'},plugins:[{name:'public-preview',setup(b){b.onLoad({filter:/app[\\/]page\.tsx$/},async args=>({contents:(await readFile(args.path,'utf8')).replace('function LandingPage(', 'export function LandingPage('),loader:'tsx'}));}}],define:{'process.env':'{}'},bundle:true,write:false,outdir:'../../output/public-preview-assets',format:'esm',platform:'browser',jsx:'automatic',logLevel:'error'});
