...
const bundle = await build({stdin:{contents:`import React from 'react'; import {createRoot} from 'react-dom/client'; import {LandingPage} from './app/page'; import DemoPage from './app/demo/page'; import HelpPage from './app/help/page'; import PricingPage from './app/pricing/page'; createRoot(document.getElementById('root')).render(location.pathname === '/demo' ? <DemoPage/> : location.pathname === '/help' ? <HelpPage/> : location.pathname === '/pricing' ? <PricingPage/> : <LandingPage start={()=>alert('Preview only. Account access is available on vanteloq.com.')}/>);`,resolveDir:process.cwd(),loader:'tsx'},plugins:[{name:'public-preview',setup(b){b.onLoad({filter:/app[\\/]page\.tsx$/},async args=>({contents:(await readFile(args.path,'utf8')).replace('function LandingPage(', 'export function LandingPage('),loader:'tsx'}));}}],define:{'process.env':'{}'},bundle:true,write:false,format:'esm',platform:'browser',jsx:'automatic',logLevel:'error'});
